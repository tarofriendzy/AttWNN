...
def get_icfg(file_name, output_file):
    if not os.path.exists(file_name):
        raise Exception('filename error')
    output_dir = os.path.dirname(output_file)
    if not os.path.exists(output_dir):
    ...
    cfg = project.analyses.CFGFast()
    icfg_graph = cfg.graph

    writer = open(output_file, 'w')
    graph_num = len(list(icfg_graph.nodes()))

    nodes = {}
    nodes_des = {}
    node_id = 0
    for node in icfg_graph.nodes:
        nodes[node] = node_id
        nodes_des[node_id] = node
        node_id += 1
    ...
    for node_id in range(len(nodes.keys())):
        ...
        successors = icfg_graph.successors(node)
        edges = []
        for succ in successors:
            edges.append(nodes[succ])
        edges_info.append(edges)

    func_json = {'node_num': graph_num,
                 'edges': edges_info,
                 'nodes': nodes_info}
    writer.write(json.dumps(func_json) + '\n')
...
def load_dataset():
    output_dir = '../data/output/xxx.json'
    graphs = read_cfg(output_dir)
...
if __name__ == '__main__':
    ...
    for i in range(len(so_files)):
        print(so_files[i])
        try:
            get_icfg(so_files[i], output_files[i] + '.json')
        except:
